...
export default function PosterPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const updateScale = () => {
      // Bulletproof scaling using window dimensions to avoid flexbox collapsing bugs
      const w = window.innerWidth - 32; // account for p-4 (16px * 2)
      const h = window.innerHeight - 240; // account for headers, template bar, and action buttons
      
      if (w > 0 && h > 0) {
        const scaleX = w / 360;
        const scaleY = h / 640;
        // Scale to 95% of available space to keep it neat but large enough
