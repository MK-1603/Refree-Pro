...
function WatchSystem({ matchDuration = 45 }: { matchDuration?: number }) {
  ...
  return (
    <group ref={watchGroup} rotation={[-0.1, 0, 0]}>
      ...
      {/* Digital display overlay — centered on watch face */}
      <Html transform position={[0, 0, 0.22]} className="pointer-events-none select-none flex flex-col items-center justify-center text-center">
        <span ref={halfTextRef} className="text-[10px] tracking-widest font-bold mb-1 text-center w-full">
          MATCH
        </span>
